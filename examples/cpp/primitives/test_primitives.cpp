...
tysoc::TSingleBody* createSingleBody( const std::string& name, const std::string& type )
{
...
int main()
{
    ...
    for ( size_t i = 0; i < NUM_BOXES; i++ )
        _scenario->addSingleBody( createSingleBody( std::string( "box_" ) + std::to_string( i ), "box" ) );

    for ( size_t i = 0; i < NUM_SPHERES; i++ )
        _scenario->addSingleBody( createSingleBody( std::string( "sphere_" ) + std::to_string( i ), "sphere" ) );

    for ( size_t i = 0; i < NUM_CYLINDERS; i++ )
        _scenario->addSingleBody( createSingleBody( std::string( "cylinder_" ) + std::to_string( i ), "cylinder" ) );

    for ( size_t i = 0; i < NUM_CAPSULES; i++ )
        _scenario->addSingleBody( createSingleBody( std::string( "capsule_" ) + std::to_string( i ), "capsule" ) );

    for ( size_t i = 0; i < NUM_MESHES; i++ )
        _scenario->addSingleBody( createSingleBody( std::string( "mesh_" ) + std::to_string( i ), "mesh" ) );
